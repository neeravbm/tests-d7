...
/**
 * Class LegalContributorUserTest
 *
 * @task SHER-53
 * @test SHER-74
 * @package tests\phpunit_tests\custom\tests\Accesslog\ServiceAccess
 */
class LegalContributorUserTest extends \PHPUnit_Framework_TestCase {
  ...
  /**
   * Legal Contributor user name.
   */
  CONST LEGAL_CONTRIBUTOR_USER = 'jbellavance';
  ...
  public static function setUpBeforeClass() {
    // Log in as a Legal Contributor user.
    db_truncate('accesslog')->execute();

    self::createContractAsAdministratorUser();

    $userObject = User::loginProgrammatically(self::LEGAL_CONTRIBUTOR_USER);
  ...
  public function testAllFiltersResponse() {
    $userObject = User::loginProgrammatically(self::ADMINISTRATOR_USER);
    $options = array(
      'userid' => self::LEGAL_CONTRIBUTOR_USER,
      'timestamp' => array(
        'min' => '-1 day',
        'max' => '1 day',
      )
